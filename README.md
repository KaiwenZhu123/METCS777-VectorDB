...
Run the RAG application (with streamlit frontend), you have access to a prompt input and filters. After a query is enter, the RAG agent will start invoking a `InformationRetriever` which calls the Weaviate database (like we did in 1) and return the chunks as `page_content`
